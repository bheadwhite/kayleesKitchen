export const shouldNotSubmitAndFocusInputs = (values, controller, change) => {
  ...
  if (activeElement.type === "text") {
    if (activeElement.name?.match(/name|amount/gi)) {
      ...
      return true;
    } else if (activeElement.name.match(/section/gi)) {
      ...
      return true;
    } else if (activeElement.name.match(/nextStep/gi)) {
      const addStep = document.getElementById("add-step");
      const name = activeElement.name;
      const index = Number(
        activeElement.name.slice(activeElement.name.indexOf("-") + 1),
      );
      if (addStep != null) {
