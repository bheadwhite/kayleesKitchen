import { Directions, Ingredient } from "src/types/types";
import StatefulSubject, { Subscription } from "src/utils/StatefulSubject";

const default_editIngredient: Ingredient = {
  name: "",
  amount: "",
  optional: false,
  unique: false,
...
export default class RecipeController {
  id = "";
  title = "";
  directions = new StatefulSubject<Directions>([]);
  ingredients = new StatefulSubject<Ingredient[]>([]);
  imageUrl = new StatefulSubject<string | null>(null);
  imageFile = new StatefulSubject<string | null>(null);
  editIngredient = new StatefulSubject<Ingredient>(default_editIngredient);
  editSection = new StatefulSubject<number>(0);
  loadingRecipeImage = new StatefulSubject<boolean>(false);
  ...
  moveStepDownOne(sectionIndex, indexOfStep) {
    if (indexOfStep === this.getDirections()[sectionIndex].steps.length - 1)
      return;
    const clone = this.getDirections().slice();
    const [removedStep] = clone[sectionIndex].steps.splice(indexOfStep, 1);
    clone[sectionIndex].steps.splice(indexOfStep + 1, 0, removedStep);
  ...
  deleteSection(sectionIndex: number | null) {
    const clone = this.getDirections().slice();
  ...
  clearEditStep(sectionIndex) {
    const clone = this.getDirections().slice();
    clone[sectionIndex].editStep = undefined;
    this.directions.next(clone);
  }
  removeImage() {
  ...
  onImageUrlChange(cb: (url: string | null) => void): Subscription {
    return this.imageUrl.subscribe({
      next: cb,
    });
  }

  onIngredientsChange(cb: (ingredients: Ingredient[]) => void): Subscription {
    return this.ingredients.subscribe({
      next: cb,
    });
  }

  onEditIngredientChange(cb: (ingredient: Ingredient) => void): Subscription {
    return this.editIngredient.subscribe({
      next: cb,
    });
  }

  onDirectionsChange(cb: (directions: Directions) => void): Subscription {
    return this.directions.subscribe({
      next: cb,
    });
  }

  onEditSectionChange(cb: (sectionIndex: number) => void): Subscription {
    return this.editSection.subscribe({
      next: cb,
    });
  }

  onLoadingRecipeImage(cb: (isLoading: boolean) => void): Subscription {
